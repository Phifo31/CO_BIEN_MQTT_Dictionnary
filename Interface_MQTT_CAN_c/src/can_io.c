 ...
 */
bool
can_init (can_ctx_t *c, const char *ifname)
{
  if (!c || !ifname)
    return false;
  memset (c, 0, sizeof (*c));
  ...
  if (fd < 0)
    {
      LOGE ("socket(PF_CAN): %s", strerror (errno));
      return false;
    }
 ...
 */
void
can_poll (can_ctx_t *c, const table_t *t, mqtt_ctx_t *m, int max_frames)
{
  if (!c || c->fd < 0)
    return;
  if (max_frames <= 0)
